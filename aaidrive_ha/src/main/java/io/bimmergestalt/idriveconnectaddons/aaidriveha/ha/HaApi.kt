...
class HaApiConnection(val wsURI: URI, private val authState: AuthState): HaApi {
    companion object {
        fun create(httpUri: String, authState: AuthState): HaApiConnection {
            return HaApiConnection(HaWsClient.parseUri(httpUri), authState)
        }
    }
    inner class Callback: HaWsClient.Callback {
        override fun onMessage(message: JSONObject) {
            when(message.getString("type")) {
                "auth_required" -> onAuthRequired()
                "auth_ok" -> connectingFlow.tryEmit(this@HaApiConnection)
    ...
    private val client = HaWsClient(wsURI, this.Callback())
    private var clientUsed = false
    private var connectingFlow = MutableSharedFlow<HaApiConnection?>(1, onBufferOverflow = BufferOverflow.DROP_OLDEST)
    ...
    fun connect(): Flow<HaApiConnection?> {
        if (!isConnected) {
            if (!clientUsed) {
                clientUsed = true
                client.connect()
